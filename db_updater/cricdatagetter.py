...
with open("completedmatches.txt", "r") as f:
    compmatches = f.read()
...
countries = {'ENG': "v England", 'IND': "v India", 'AUS': "v Australia", 'PAK': "v Pakistan", 'WI': "v West Indies",
               'SRI': "v Sri Lanka", 'AFG': "v Afghanistan", 'IRE': "v Ireland", 'RSA': "v South Africa", 'ZIM':
                   "v Zimbabwe", 'BAN': "Bangladesh", 'NZ': "v New Zealand"}
...
for matchnum in matches:
    ...
    for i in range(4):
        for row in m.all_innings[i]['batsmen']:
            ...
            c.execute("INSERT INTO Test (InningsPlayer, InningsRunsScored, InningsRunsScoredNum,"
    ...
    for i in range(4):
        for row in m.all_innings[i]['didNotBat']:
            if row != None:
                name = row['name']
                c.execute(
                    "INSERT INTO Test (InningsPlayer, InningsRunsScored, InningsBattedFlag, InningsNotOutFlag,"
                    " InningsNumber, Opposition, Ground, InningsDate, Country, '50s', '100s') "
                    "VALUES (?,?,?,?,?,?,?,?,?,?,?)", [name, "DNB", 0, 0, i + 1, opp, ground, date, country, 0, 0])
    ...
    for i in range(4):
        ...
        for row in m.all_innings[i]['bowlers']:
            ...
            c.execute("INSERT INTO Test (InningsPlayer,InningsNumber,Opposition, Ground, InningsDate, Country,"
                      ...
                      " InningsWicketsTakenBuckets, InningsEconomyRate) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
                      [name, i + 1, opp, ground, date, country, overs, 1, maidens, conceded, wickets, wick4, wick5,
                       wick10, buck, eco])
        ...
        for name in nameset:
            if name not in bowlers:
                c.execute("INSERT INTO Test (InningsPlayer, InningsNumber, Opposition, Ground, InningsDate, Country,"
                          " InningsOversBowled, InningsBowledFlag, '4wickets', '5wickets', '10wickets') VALUES "
                          "(?,?,?,?,?,?,?,?,?,?,?)",
                          [name, i + 1, opp, ground, date, country, 'DNB', 0, 1, 0, 0])

    with open("completedmatches.txt", "a") as f:
